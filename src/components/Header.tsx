...
export default function Header() {
  ...
  const displayName = profile?.full_name?.split(' ').slice(0, 2).join(' ') || '';
            ...
            {showMenu && (
              <div className="absolute right-0 top-12 w-[180px] sm:w-[200px] card-base overflow-hidden z-50">
                <div className="px-4 py-3 border-b border-[var(--border-subtle)]">
                  <p className="font-dm text-sm font-semibold text-[var(--text-primary)] truncate">{profile?.full_name}</p>
                  <p className="font-mono text-xs text-[var(--text-tertiary)]">{profile?.email}</p>
                </div>
