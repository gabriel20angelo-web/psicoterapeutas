...
export default function PatientProfilePage() {
  ...
  const [sessionNotes, setSessionNotes] = useState<SessionNote[]>(() => paciente ? loadSessionNotes(paciente.id) : []);
  const [preSessionNotes, setPreSessionNotes] = useState<SessionNote[]>(() => paciente ? loadPreSessionNotes(paciente.id) : []);
  ...
  const [generalNote, setGeneralNote] = useState(() => paciente ? loadGeneralNote(paciente.id) : '');
  ...
  useEffect(() => {
    // Track if general note has changed from its saved value
    const savedValue = paciente ? loadGeneralNote(paciente.id) : '';
    setIsDirty(generalNote !== savedValue || editingId !== null);
  }, [generalNote, editingId, paciente]);
  ...
  const handleGeneralNoteChange = useCallback((value: string) => {
    ...
    autoSaveTimerRef.current = setTimeout(() => {
      if (paciente) {
        ...
        saveGeneralNote(paciente.id, value);
        setTimeout(() => {
  ...
  const saveSessionNote = (sid: string, date: string, content: string) => { const updated = saveSessionNoteLib(paciente.id, sid, date, content); setSessionNotes(updated); setEditingId(null); };
  const savePreNote = (sid: string, date: string, content: string) => { const updated = savePreNoteLib(paciente.id, sid, date, content); setPreSessionNotes(updated); setEditingPreId(null); };
  const handleSaveGeneral = () => { saveGeneralNote(paciente.id, generalNote); setGeneralSaved(true); setTimeout(() => setGeneralSaved(false), 2000); };
  ...
  const handleExportPDF = () => {
    const generalNoteContent = loadGeneralNote(paciente.id);
