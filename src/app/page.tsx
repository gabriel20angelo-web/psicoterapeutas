...
import { ArrowRight, CheckCircle2, Send, ChevronDown, FileText, BookOpen, Flame, Sparkles, AlertTriangle } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const handleSaveNote = async (a: Atividade) => {
    if (a.paciente && noteContent.trim()) {
      await saveSessionNoteLib(a.paciente.id, a.id, a.data_inicio, noteContent.trim());
    }
    setNoteSessionId(null);
    setNoteContent("");
  };
  ...
  return (
    <Shell>
      ...
      {/* Week stats */}
      <motion.div {...staggerChild(3.5)} className="mb-6">
        <Card>
            ...
            <Card className="mb-6">
              ...
              <div className="mb-3">
                ...
                <div className="w-full h-2 rounded-full bg-[var(--bg-hover)]">
                  <div className="h-2 rounded-full bg-[var(--orange-500)] transition-all" style={{ width: `${progresso}%` }} />
                </div>
              </div>
      ...
      {/* Acadêmico Widget */}
      {(() => {
        const acAtivo = getModulosAtivos().some(m => m.id === "academico");
        if (!acAtivo) return null;
        const acStats = getAcademicoStats();
        const acTarefas = getAcTarefasProximas(7).slice(0, 3);
        if (acStats.disciplinasCursando === 0 && acTarefas.length === 0) return null;
        return (
          <motion.div {...staggerChild(5.8)}>
            <Card className="mb-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <GraduationCap size={18} className="text-[var(--orange-500)]" />
                  <h2 className="font-fraunces font-bold text-lg text-[var(--text-primary)]">Acadêmico</h2>
                </div>
                <Link href="/academico" className="font-dm text-xs text-[var(--orange-500)] hover:underline flex items-center gap-1">
                  Abrir <ArrowRight size={12} />
                </Link>
              </div>
              <div className="flex items-center gap-4 mb-3 flex-wrap">
                <div className="font-dm text-xs text-[var(--text-secondary)]">
                  <span className="font-mono font-bold text-[var(--text-primary)]">{acStats.disciplinasCursando}</span> disciplinas
                </div>
                <div className="font-dm text-xs text-[var(--text-secondary)]">
                  <span className="font-mono font-bold text-[var(--text-primary)]">{acStats.tarefasPendentes}</span> pendentes
                </div>
                <div className="font-dm text-xs text-[var(--text-secondary)]">
                  <span className="font-mono font-bold text-[var(--orange-500)]">{acStats.progressoLeitura.lidos}/{acStats.progressoLeitura.meta}</span> livros
                </div>
              </div>
              {acStats.disciplinasEmRisco.length > 0 && (
                <div className="mb-3">
                  {acStats.disciplinasEmRisco.map(({ disciplina, resumo }) => (
                    <div key={disciplina.id} className="flex items-center gap-2 px-2 py-1 rounded-lg bg-[var(--red-bg)] mb-1">
                      <AlertTriangle size={12} className="text-[var(--red-text)]" />
                      <span className="font-dm text-xs text-[var(--red-text)]">{disciplina.nome}: {resumo.percentual}% freq.</span>
                    </div>
                  ))}
                </div>
              )}
              {acTarefas.length > 0 && (
                <div className="space-y-2">
                  {acTarefas.map(t => (
                    <div key={t.id} className="flex items-center justify-between py-1.5 border-b border-[var(--border-default)] last:border-0">
                      <span className="font-dm text-sm text-[var(--text-primary)] truncate">{t.titulo}</span>
                      <span className={`font-mono text-[10px] flex-shrink-0 ${t.data_entrega <= new Date().toISOString().slice(0, 10) ? 'text-[var(--red-text)]' : 'text-[var(--text-tertiary)]'}`}>
                        {t.data_entrega}
                      </span>
                    </div>
                  ))}
